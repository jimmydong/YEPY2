...
def dump(o, space, num, key, typ, proret):
    # 防止循环
    if space >= (max_level*TAB_SIZE):
        return display(' ... reach max_level', space, num, key, typ, proret)
    try:
      if o in recorder:
        return display(' ... recursion variable', space, num, key, typ, proret)
    except TypeError:  
        return display(' ... unhash variable', space, num, key, typ, proret)
